...
function setup() {
  ...
  var canvas = createCanvas(1480, 920);
...
function Gui() {
  var guiA = createP("LESS TRAILS WHEN MOVING");
  guiA.size(130).style("overflow", "auto");
  guiA.position(10, 210)
  guiA.addClass("gui");
  slider = createSlider(20, 255, 20);
  slider.position(0, 200);

  var guiB = createP("SCALE");
  guiB.position(10, 310);
  guiB.addClass("gui");
  sliderscaleUpSensor = createSlider(1, 10, 3);
  sliderscaleUpSensor.position(0, 300);

  var guiC = createP("MOVE LEFT/RIGHT");
  guiC.position(10, 410)
  guiC.addClass("gui");
  sliderLR = createSlider(-800, 800, 0);
  sliderLR.position(0, 400);

  var guiD = createP("THICKNESS");
  guiD.position(10, 510)
  guiD.addClass("gui");
  sliderThick = createSlider(0, 12, 5);
  sliderThick.position(0, 500);
...
function createOneButton() {
  var bottomrightP = createButton('SAVE');
  bottomrightP.position(10, 600);
  bottomrightP.addClass("instructions")
